...
class DATA_LOADER(object):
	...
	def read_matdataset(self, opt):
		matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.image_embedding + ".mat")#resnet101
		...
		# self.graph_attall = torch.load("./raw_graphAtt.pt")
		if not opt.validation:
		...
		self.train_samples_class_index = torch.tensor([self.train_label.eq(i_class).sum().float() for i_class in self.train_class])# 计算每个训练类有多少样本
